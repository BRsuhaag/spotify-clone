...
async function getSongs(folder) {
    ...
    Array.from(songUL.getElementsByTagName("li")).forEach((e, index) => {
        e.addEventListener("click", () => {
            const songName = songs[index]; // Use the original song name from the array
...
async function displayAlbum() {
    ...
    Array.from(document.getElementsByClassName("card")).forEach((e) => {
        e.addEventListener("click", async (item) => {
            ...
            if (songs.length > 0) {
                playMusic(songs[0], true); // Play first song without auto-start
            }
        });
    });
...
async function main() {
    ...
    previous.addEventListener("click", () => {
        ...
        let currentSrc = decodeURIComponent(currentSong.src.split("/").pop()); // Properly decode and get current song name
    ...
    next.addEventListener("click", () => {
        ...
        let currentSrc = decodeURIComponent(currentSong.src.split("/").pop()); // Properly decode and get current song name
        let index = songs.indexOf(currentSrc);
        if (index < songs.length - 1) { // Prevent going out of bounds
            playMusic(songs[index + 1]);
        }
    });
    ...
    document.querySelector(".volume > img").addEventListener("click", (e) => {
        if (e.target.src.includes("volume.svg")) {
            ...
            currentSong.volume = 0.5; // Default volume on unmute
